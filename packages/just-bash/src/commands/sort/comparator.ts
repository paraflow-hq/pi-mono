 ...
 */
function extractKeyValue(line: string, key: KeySpec, delimiter: string | null): string {
	...
	for (let i = startFieldIdx; i <= endFieldIdx && i < fields.length; i++) {
		...
		if (i === endFieldIdx && key.endChar !== undefined) {
			...
			const endIdx =
				i === startFieldIdx && key.startChar !== undefined ? key.endChar - key.startChar + 1 : key.endChar;
 ...
 */
export function createComparator(options: SortOptions): (a: string, b: string) => number {
	const {
		keys,
		fieldDelimiter,
