...
import { buildRegex, convertReplacement, type RegexResult, searchContent } from "../search-engine/index.js";
 ...
 */
export async function executeSearch(searchCtx: SearchContext): Promise<ExecResult> {
 ...
 */
function buildSearchRegex(patterns: string[], options: RgOptions, ignoreCase: boolean): RegexResult {
	let combinedPattern: string;
	if (patterns.length === 1) {
		combinedPattern = patterns[0];
	} else {
		combinedPattern = patterns
			.map((p) => (options.fixedStrings ? p.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") : `(?:${p})`))
			.join("|");
	}
 ...
 */
async function listFiles(ctx: CommandContext, inputPaths: string[], options: RgOptions): Promise<ExecResult> {
	// Load gitignore files
	let gitignore: GitignoreManager | null = null;
	if (!options.noIgnore) {
		gitignore = await loadGitignores(ctx.fs, ctx.cwd, options.noIgnoreDot, options.noIgnoreVcs, options.ignoreFiles);
