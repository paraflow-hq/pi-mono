 ...
 */
function applyValueOpt(options: RgOptions, def: ValueOptDef, value: string): ExecResult | undefined {
	if (def.validate) {
 ...
 * Parse sort option
 */
function parseSort(args: string[], i: number): { value: "path" | "none"; newIndex: number } | null {
	const arg = args[i];
