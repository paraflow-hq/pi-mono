...
function applyReplacement(replacement: string, match: RegExpExecArray): string {
	return replacement.replace(/\$(&|\d+|<([^>]+)>)/g, (_, ref: string, namedGroup: string | undefined) => {
		if (ref === "&") {
			return match[0];
		}
 ...
 * - Max count (-m)
 */
export function searchContent(content: string, regex: UserRegex, options: SearchOptions = {}): SearchResult {
	const {
		invertMatch = false,
		...
		for (let i = 0; i < lastIdx; i++) {
			...
			if (matches !== invertMatch) {
				...
				if (onlyMatching) {
					...
					let prefix = filename ? `${filename}:` : "";
					if (showByteOffset) prefix += `${byteOffset + (firstMatch ? firstMatch.index : 0)}:`;
					if (showLineNumbers) prefix += `${i + 1}:`;
		...
		if (onlyMatching) {
			...
			for (let i = span.startLine; i <= span.endLine && i < lastIdx; i++) {
				if (!printedLines.has(i)) {
					...
					let prefix = filename ? `${filename}:` : "";
					if (showByteOffset && i === span.startLine) prefix += `${span.byteOffset}:`;
					if (showLineNumbers) prefix += `${i + 1}:`;
