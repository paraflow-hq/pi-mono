...
function isRelativeOffset(address: SedAddress): address is import("./types.js").RelativeOffset {
...
function processReplacement(replacement: string, match: string, groups: string[]): string {
	let result = "";
	let i = 0;
...
function executeCommand(cmd: SedCommand, state: SedState): void {
	...
	switch (cmd.type) {
				...
				if (hasMatch) {
					...
					if (subCmd.nthOccurrence && subCmd.nthOccurrence > 0 && !subCmd.global) {
						...
						state.patternSpace = globalReplace(
							...
							globalRegex,
							subCmd.replacement,
							(match, groups) => processReplacement(subCmd.replacement, match, groups),
						);
					} else {
		...
		case "transliterate":
			...
			state.patternSpace = executeTransliterate(state.patternSpace, cmd as TransliterateCommand);
			break;
...
function executeTransliterate(input: string, cmd: TransliterateCommand): string {
	let result = "";
