...
export class SedLexer {
	...
	private makeToken(type: SedTokenType, value: string | number, extra?: Partial<SedToken>): SedToken {
	...
	private readTextCommand(cmd: string, startLine: number, startColumn: number): SedToken {
	...
	private readBranch(type: SedTokenType, cmd: string, startLine: number, startColumn: number): SedToken {
		// Skip whitespace
		while (this.peek() === " " || this.peek() === "\t") {
			this.advance();
		}
	...
	private readFileCommand(type: SedTokenType, cmd: string, startLine: number, startColumn: number): SedToken {
