...
class SedParser {
	...
	private parseSimpleCommand(token: SedToken, address?: AddressRange): { command: SedCommand | null; error?: string } {
		this.advance();
	...
	private parseTextCommand(token: SedToken, address?: AddressRange): { command: SedCommand | null; error?: string } {
		const cmd = token.value as string;
...
function findUndefinedLabel(commands: SedCommand[], definedLabels: Set<string>): string | undefined {
	for (const cmd of commands) {
		if (
			(cmd.type === "branch" || cmd.type === "branchOnSubst" || cmd.type === "branchOnNoSubst") &&
			cmd.label &&
