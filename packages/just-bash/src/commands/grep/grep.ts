		...
		for (let i = 0; i < args.length; i++) {
			...
			if (arg.startsWith("-") && arg !== "-") {
				...
				for (const flag of flags) {
					...
					else if (flag === "L" || flag === "--files-without-match") filesWithoutMatch = true;
 ...
 */
async function expandGlobPatternWithTypes(pattern: string, ctx: CommandContext): Promise<FileEntry[]> {
