...
async function computeHash(algorithm: HashAlgorithm, data: Uint8Array): Promise<string> {
	if (algorithm === "md5") {
		return md5(data);
	}
	...
	const hashBuffer = await globalThis.crypto.subtle.digest(algoName, new Uint8Array(data).buffer as ArrayBuffer);
	return Array.from(new Uint8Array(hashBuffer))
		.map((b) => b.toString(16).padStart(2, "0"))
		.join("");
}

export function createChecksumCommand(name: string, algorithm: HashAlgorithm, summary: string): Command {
	const help = {
		name,
		summary,
		usage: `${name} [OPTION]... [FILE]...`,
		options: ["-c, --check    read checksums from FILEs and check them", "    --help     display this help and exit"],
	};
		...
		async execute(args: string[], ctx: CommandContext): Promise<ExecResult> {
			...
			for (const arg of args) {
				if (arg === "-c" || arg === "--check") check = true;
					...
					/* ignored */
				} else if (arg.startsWith("-") && arg !== "-") return unknownOption(name, arg);
				else files.push(arg);
			}
			...
			if (check) {
				...
				for (const file of files) {
					...
					const content =
						file === "-" ? ctx.stdin : await ctx.fs.readFile(ctx.fs.resolvePath(ctx.cwd, file)).catch(() => null);
					if (content === null)
						return {
							stdout: "",
