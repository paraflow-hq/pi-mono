...
const validInputFormats = ["yaml", "xml", "json", "ini", "csv", "toml"] as const;
const validOutputFormats = ["yaml", "json", "xml", "ini", "csv", "toml"] as const;
...
export function isValidInputFormat(value: unknown): value is InputFormat {
	return typeof value === "string" && validInputFormats.includes(value as InputFormat);
}
...
export function isValidOutputFormat(value: unknown): value is OutputFormat {
	return typeof value === "string" && validOutputFormats.includes(value as OutputFormat);
}
 ...
 */
function parseCsv(input: string, delimiter: string, hasHeader: boolean): unknown[] {
 ...
 */
export function extractFrontMatter(input: string): { frontMatter: QueryValue; content: string } | null {
 ...
 */
export function formatOutput(value: QueryValue, options: FormatOptions): string {
