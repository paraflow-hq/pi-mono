 ...
 */
function generateSuffix(index: number, useNumeric: boolean, length: number): string {
	if (useNumeric) {
 ...
 */
function splitByLines(content: string, linesPerFile: number): { content: string; hasContent: boolean }[] {
 ...
 */
function splitByBytes(content: string, bytesPerFile: number): { content: string; hasContent: boolean }[] {
 ...
 */
function splitIntoChunks(content: string, numChunks: number): { content: string; hasContent: boolean }[] {
	const encoder = new TextEncoder();
	const bytes = encoder.encode(content);
	const decoder = new TextDecoder();
	const chunks: { content: string; hasContent: boolean }[] = [];
