 ...
 */
export function formatStrftime(format: string, timestamp: number, tz?: string): string {
...
function getDatePartsInTimezone(
	...
	try {
		...
		const getValue = (type: string): string => parts.find((p) => p.type === type)?.value ?? "";
 ...
 */
function formatStrftimeDirective(date: Date, directive: string, tz?: string): string | null {
	...
	switch (directive) {
		...
		case "A":
			return ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"][parts.weekday];
		case "b":
		case "h":
			return ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][parts.month - 1];
		case "B":
			return [
				"January",
				"February",
		...
		case "c":
			return `${["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][parts.weekday]} ${
				["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][parts.month - 1]
 ...
 */
function getDayOfYearForParts(year: number, month: number, day: number): number {
 ...
 */
function getWeekNumberForParts(year: number, month: number, day: number, weekday: number, startDay: number): number {
 ...
 */
function getISOWeekNumberForParts(year: number, month: number, day: number): number {
