 ...
 */
export function applyWidth(value: string, width: number, precision: number): string {
	let result = value;
 ...
 */
export function parseWidthPrecision(format: string, startIndex: number): [number, number, number] {
	let i = startIndex;
	let width = 0;
	let precision = -1;
