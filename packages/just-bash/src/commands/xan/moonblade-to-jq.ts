...
const FUNCTION_MAP: Record<string, string | ((args: AstNode[]) => AstNode)> = {
	...
	join: (args) =>
		args.length === 1 ? makeCall("join", [{ type: "Literal", value: "" }]) : makePipeFunc("join", args),
	...
	coalesce: (args) => {
		...
		return makeCond(
			condition,
			first,
			rest.length === 1 ? rest[0] : (FUNCTION_MAP.coalesce as (args: AstNode[]) => AstNode)(rest),
		);
	},
 ...
 */
function makeCond(cond: AstNode, thenBranch: AstNode, elseBranch?: AstNode): AstNode {
...
export function moonbladeToJq(expr: MoonbladeExpr, rowContext = true): AstNode {
	switch (expr.type) {
		...
		case "func": {
			...
			const handler = Object.hasOwn(FUNCTION_MAP, expr.name) ? FUNCTION_MAP[expr.name] : undefined;
		...
		default:
			throw new Error(`Unknown moonblade expression type: ${(expr as MoonbladeExpr).type}`);
	}
}
