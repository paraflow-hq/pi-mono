...
import { type CsvData, type CsvRow, createSafeRow, formatCsv, readCsvInput, safeSetRow, toSafeRow } from "./csv.js";
 ...
 */
export async function cmdExplode(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let column = "";
	let separator = "|";
	let dropEmpty = false;
	let rename = "";
 ...
 *   -r, --rename NAME    Rename the column
 */
export async function cmdImplode(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let column = "";
	let separator = "|";
	let rename = "";
	const fileArgs: string[] = [];
 ...
 */
export async function cmdJoin(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let key1 = "";
	let file1 = "";
	let key2 = "";
	let file2 = "";
 ...
 */
export async function cmdPivot(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let pivotCol = "";
	let aggExpr = "";
	...
	// Group data
	const groups = new Map<string, Map<string, (string | number | boolean | null)[]>>();
...
function computeSimpleAgg(func: string, values: (string | number | boolean | null)[]): string | number | null {
	const nums = values
 ...
 */
export async function cmdMerge(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let sortCol = "";
	const fileArgs: string[] = [];
