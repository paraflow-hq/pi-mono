 ...
 */
export async function cmdBehead(args: string[], ctx: CommandContext): Promise<ExecResult> {
 ...
 */
export async function cmdSample(args: string[], ctx: CommandContext): Promise<ExecResult> {
 ...
 */
export async function cmdCat(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let pad = false;
	const fileArgs: string[] = [];
 ...
 */
export async function cmdSearch(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let pattern = "";
 ...
 */
export async function cmdFlatmap(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let expr = "";
	const fileArgs: string[] = [];
	...
	const evalOptions: EvaluateOptions = {
		limits: ctx.limits ? { maxIterations: ctx.limits.maxJqIterations } : undefined,
	};
	...
	for (const row of data) {
		...
		for (let i = 0; i < maxLen; i++) {
			...
			for (let j = 0; j < specs.length; j++) {
				...
				safeSetRow(newRow, specs[j].alias, val as string | number | boolean | null);
			}
			newData.push(newRow);
		}
	}
 ...
 */
export async function cmdFmt(args: string[], ctx: CommandContext): Promise<ExecResult> {
