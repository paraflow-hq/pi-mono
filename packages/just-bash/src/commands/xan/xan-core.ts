...
export async function cmdHeaders(args: string[], ctx: CommandContext): Promise<ExecResult> {
...
export async function cmdCount(args: string[], ctx: CommandContext): Promise<ExecResult> {
	const { data, error } = await readCsvInput(args, ctx);
	if (error) return error;
	return { stdout: `${data.length}\n`, stderr: "", exitCode: 0 };
}

export async function cmdHead(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let n = 10;
...
export async function cmdTail(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let n = 10;
	const filteredArgs: string[] = [];
	for (let i = 0; i < args.length; i++) {
		if ((args[i] === "-l" || args[i] === "-n") && i + 1 < args.length) {
...
export async function cmdSlice(args: string[], ctx: CommandContext): Promise<ExecResult> {
...
export async function cmdReverse(args: string[], ctx: CommandContext): Promise<ExecResult> {
