...
import { type Token, Tokenizer, type TokenType } from "./moonblade-tokenizer.js";
...
class Parser {
	...
	private handleIndexing(left: MoonbladeExpr): MoonbladeExpr {
		...
		if (this.peek().type === ":") {
			...
			return {
				type: "func",
				name: "slice",
				args: [{ expr: left }, { expr: { type: "int", value: 0 } }, { expr: end }],
			};
		}
	...
	private fillUnderscore(expr: MoonbladeExpr, fill: MoonbladeExpr): MoonbladeExpr {
	...
	private bindLambdaArgsInExpr(expr: MoonbladeExpr, names: string[]): MoonbladeExpr {
...
export function parseNamedExpressions(input: string): NamedExpr[] {
	...
	while (peek().type !== "eof") {
		...
		while (peek().type !== "eof") {
			...
			if (token.type === "(" || token.type === "[" || token.type === "{") depth++;
			if (token.type === ")" || token.type === "]" || token.type === "}") depth--;
