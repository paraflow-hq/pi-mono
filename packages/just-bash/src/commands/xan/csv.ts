 ...
 */
export function safeSetRow(row: CsvRow, key: string, value: string | number | boolean | null): void {
	row[key] = value;
}
...
export function parseCsv(input: string): { headers: string[]; data: CsvData } {
	...
	const safeData = result.data.map((row) => toSafeRow(row as Record<string, unknown>));
	return {
