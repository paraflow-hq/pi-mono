...
import { type CsvData, type CsvRow, formatCsv, readCsvInput, safeSetRow, toSafeRow } from "./csv.js";
import { parseNamedExpressions } from "./moonblade-parser.js";
import { moonbladeToJq } from "./moonblade-to-jq.js";

export async function cmdMap(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let mapExpr = "";
	let overwrite = false;
	let filter = false;
 ...
 */
export async function cmdTransform(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let targetCol = "";
	let transformExpr = "";
	let rename = "";
	const fileArgs: string[] = [];
	...
	const ast = moonbladeToJq(
		parseNamedExpressions(transformExpr)[0]?.expr || require("./moonblade-parser.js").parseMoonblade(transformExpr),
	);
