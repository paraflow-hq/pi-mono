...
import { type CsvData, type CsvRow, createSafeRow, formatCsv, readCsvInput, safeSetRow } from "./csv.js";
 ...
 */
export async function cmdTranspose(args: string[], ctx: CommandContext): Promise<ExecResult> {
	...
	const newHeaders = [firstCol, ...data.map((row, i) => String(row[firstCol] ?? `row_${i}`))];
 ...
 */
export async function cmdShuffle(args: string[], ctx: CommandContext): Promise<ExecResult> {
 ...
 */
export async function cmdFixlengths(args: string[], ctx: CommandContext): Promise<ExecResult> {
 ...
 */
export async function cmdSplit(args: string[], ctx: CommandContext): Promise<ExecResult> {
 ...
 *   -o, --output DIR  Output directory (default: current)
 */
export async function cmdPartition(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let column = "";
	let outputDir = ".";
	const fileArgs: string[] = [];
 ...
 *   FORMAT: json
 */
export async function cmdTo(args: string[], ctx: CommandContext): Promise<ExecResult> {
	if (args.length === 0) {
		return {
			stdout: "",
 ...
 */
async function cmdToJson(args: string[], ctx: CommandContext): Promise<ExecResult> {
	const fileArgs = args.filter((a) => !a.startsWith("-"));
 ...
 */
export async function cmdFrom(args: string[], ctx: CommandContext): Promise<ExecResult> {
	let format = "";
	const fileArgs: string[] = [];
 ...
 */
async function cmdFromJson(fileArgs: string[], ctx: CommandContext): Promise<ExecResult> {
	const file = fileArgs[0];
	let input: string;
		...
		if (Array.isArray(data[0])) {
			...
			const csvData: CsvData = rows.map((row) => {
				...
				for (let i = 0; i < (headers as string[]).length; i++) {
					safeSetRow(obj, (headers as string[])[i], row[i] as string | number | boolean | null);
				}
				return obj;
			});
			return {
