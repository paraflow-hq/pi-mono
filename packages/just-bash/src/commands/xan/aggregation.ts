...
function evalExpr(row: CsvRow, expr: string, evalOptions: EvaluateOptions): unknown {
...
export function computeAgg(
	...
	if (isSimpleColumn(expr)) {
		values = data.map((r) => r[expr]).filter((v) => v !== null && v !== undefined);
	} else {
		// Complex expression - evaluate for each row
		values = data.map((r) => evalExpr(r, expr, evalOptions)).filter((v) => v !== null && v !== undefined);
	}
	...
	switch (func) {
		...
		case "last":
			return values.length > 0 ? (values[values.length - 1] as string | number | null) : null;
...
export function buildAggRow(data: CsvData, specs: AggSpec[], evalOptions: EvaluateOptions = {}): CsvRow {
