...
import { isSafeKey, nullPrototypeCopy, nullPrototypeMerge, safeHasOwn, safeSet } from "./safe-object.js";
import {
	compare,
	compareJq,
	containsDeep,
...
export interface EvalContext {
	...
	funcs?: Map<string, { params: string[]; body: AstNode; closure?: Map<string, unknown> }>;
	labels?: Set<string>;
...
function withVar(ctx: EvalContext, name: string, value: QueryValue): EvalContext {
 ...
 */
function bindPattern(ctx: EvalContext, pattern: DestructurePattern, value: QueryValue): EvalContext | null {
	switch (pattern.type) {
		case "var":
...
function getValueAtPath(root: QueryValue, path: (string | number)[]): QueryValue {
	let v = root;
 ...
 */
function applyPathTransform(basePath: (string | number)[], ast: AstNode): (string | number)[] | null {
	if (ast.type === "Call") {
		if (ast.name === "parent") {
 ...
 */
function evaluateWithPartialResults(value: QueryValue, ast: AstNode, ctx: EvalContext): QueryValue[] {
...
export function evaluate(value: QueryValue, ast: AstNode, ctxOrOptions?: EvalContext | EvaluateOptions): QueryValue[] {
	let ctx: EvalContext =
			...
			return operands.map((v) => {
				if (ast.op === "-") {
					...
					if (typeof v === "string") {
						...
						const formatStr = (s: string) => (s.length > 5 ? `"${s.slice(0, 3)}...` : JSON.stringify(s));
...
function applyUpdate(
	...
	function computeNewValue(current: QueryValue, newVal: QueryValue): QueryValue {
		switch (op) {
			case "=":
				return newVal;
			case "|=": {
			...
			case "+=":
				if (typeof current === "number" && typeof newVal === "number") return current + newVal;
				if (typeof current === "string" && typeof newVal === "string") return current + newVal;
				if (Array.isArray(current) && Array.isArray(newVal)) return [...current, ...newVal];
				...
				if (path.base) {
					return updateRecursive(val, path.base, (baseVal) => {
						if (baseVal && typeof baseVal === "object" && !Array.isArray(baseVal)) {
							...
							const current = Object.hasOwn(obj, path.name) ? obj[path.name] : undefined;
...
function applyDel(root: QueryValue, pathExpr: AstNode, ctx: EvalContext): QueryValue {
	// Helper to set a value at an AST path
	function setAtPath(obj: QueryValue, pathNode: AstNode, newVal: QueryValue): QueryValue {
	...
	function deleteAt(val: QueryValue, path: AstNode): QueryValue {
		switch (path.type) {
			...
			case "Pipe": {
				...
				// Helper to set a value at an AST path
				function setAt(obj: QueryValue, pathNode: AstNode, newVal: QueryValue): QueryValue {
					switch (pathNode.type) {
						case "Identity":
							return newVal;
						case "Field": {
...
function evalBinaryOp(value: QueryValue, op: string, left: AstNode, right: AstNode, ctx: EvalContext): QueryValue[] {
			...
			switch (op) {
				...
				case "-":
					...
					if (typeof l === "string" && typeof r === "string") {
						...
						// jq format: "string (\"truncated...) - no closing quote when truncated
						const formatStr = (s: string) => (s.length > 10 ? `"${s.slice(0, 10)}...` : JSON.stringify(s));
				...
				case "*":
					...
					if (l && r && typeof l === "object" && typeof r === "object" && !Array.isArray(l) && !Array.isArray(r)) {
						return deepMerge(l as Record<string, unknown>, r as Record<string, unknown>);
					}
					return null;
				case "/":
...
function evalBuiltin(value: QueryValue, name: string, args: AstNode[], ctx: EvalContext): QueryValue[] {
