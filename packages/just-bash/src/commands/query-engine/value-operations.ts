...
import { isSafeKey, nullPrototypeCopy, safeHasOwn, safeSet } from "./safe-object.js";
...
export function deepMerge(a: Record<string, unknown>, b: Record<string, unknown>): Record<string, unknown> {
	...
	for (const key of Object.keys(b)) {
		...
		if (
			...
			safeSet(result, key, deepMerge(result[key] as Record<string, unknown>, b[key] as Record<string, unknown>));
		} else {
