...
export function evalArrayBuiltin(
	...
	switch (name) {
		...
		case "max_by": {
			if (!Array.isArray(value) || value.length === 0 || args.length === 0) return [null];
			return [
				value.reduce((a, b) => {
					const aKey = evaluate(a, args[0], ctx)[0];
					const bKey = evaluate(b, args[0], ctx)[0];
		...
		case "any": {
			...
			if (args.length === 1) {
				if (Array.isArray(value)) {
					return [value.some((item) => isTruthy(evaluate(item, args[0], ctx)[0]))];
				}
				return [false];
			}
		...
		case "all": {
			...
			if (args.length === 1) {
				if (Array.isArray(value)) {
					return [value.every((item) => isTruthy(evaluate(item, args[0], ctx)[0]))];
				}
				return [true];
			}
