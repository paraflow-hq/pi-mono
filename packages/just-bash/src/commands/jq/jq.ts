...
import { type EvaluateOptions, evaluate, parse, type QueryValue } from "../query-engine/index.js";
...
function formatValue(
	...
	if (Array.isArray(v)) {
		...
		const items = v.map(
			(x) => indentStr.repeat(indent + 1) + formatValue(x, false, false, sortKeys, useTab, indent + 1),
		);
	...
	if (typeof v === "object") {
		...
		const items = keys.map((k) => {
			// @banned-pattern-ignore: iterating via Object.keys() which only returns own properties
			const val = formatValue((v as Record<string, unknown>)[k], false, false, sortKeys, useTab, indent + 1);
...
export const jqCommand: Command = {
	...
	async execute(args: string[], ctx: CommandContext): Promise<ExecResult> {
		...
		try {
			...
			if (maxStringLength !== undefined && maxStringLength > 0 && output.length > maxStringLength) {
				throw new ExecutionLimitError(`jq: output size limit exceeded (${maxStringLength} bytes)`, "string_length");
			}

			const exitCode =
				exitStatus && (values.length === 0 || values.every((v) => v === null || v === undefined || v === false))
					? 1
					: 0;
