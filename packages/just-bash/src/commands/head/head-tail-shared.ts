...
export type HeadTailParseResult = { ok: true; options: HeadTailOptions } | { ok: false; error: ExecResult };

/**
 * Parse head/tail command arguments.
 * Both commands share most options, with tail having additional +N syntax.
 */
export function parseHeadTailArgs(args: string[], cmdName: "head" | "tail"): HeadTailParseResult {
	let lines = 10;
 ...
 */
export function getHead(content: string, lines: number, bytes: number | null): string {
	if (bytes !== null) {
 ...
 */
export function getTail(content: string, lines: number, bytes: number | null, fromLine: boolean): string {
	if (bytes !== null) {
