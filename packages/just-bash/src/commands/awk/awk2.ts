...
import { type AwkFileSystem, AwkInterpreter, createRuntimeContext } from "./interpreter/index.js";
...
export const awkCommand2: Command = {
	...
	async execute(args: string[], ctx: CommandContext): Promise<ExecResult> {
		...
		let fieldSep: import("../../regex/index.js").RegexLike = new ConstantRegex(/\s+/);
		let fieldSepStr = " ";
		...
		const runtimeCtx = createRuntimeContext({
			...
			exec: ctx.exec ? (cmd: string) => ctx.exec!(cmd, { cwd: ctx.cwd }) : undefined,
...
function createFieldSepRegex(sep: string): import("../../regex/index.js").UserRegex {
	if (sep === " ") {
