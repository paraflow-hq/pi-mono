 ...
 */
async function extractPatternArg(arg: AwkExpr, evaluator: AwkEvaluator): Promise<string> {
	if (arg.type === "regex") {
		return arg.pattern;
	}
 ...
 */
async function resolveTargetName(targetExpr: AwkExpr | undefined, evaluator: AwkEvaluator): Promise<string> {
 ...
 */
function applyTargetValue(targetName: string, newValue: string, ctx: AwkRuntimeContext): void {
	if (targetName === "$0") {
		ctx.line = newValue;
...
async function awkLength(args: AwkExpr[], ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length === 0) {
...
async function awkSubstr(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<string> {
...
async function awkIndex(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
...
async function awkSplit(args: AwkExpr[], ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length < 2) return 0;
	const str = toAwkString(await evaluator.evalExpr(args[0]));
...
async function awkSub(args: AwkExpr[], ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length < 2) return 0;
...
async function awkGsub(args: AwkExpr[], ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length < 2) return 0;
...
async function awkMatch(args: AwkExpr[], ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length < 2) {
		ctx.RSTART = 0;
		ctx.RLENGTH = -1;
		return 0;
...
async function awkGensub(args: AwkExpr[], ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<string> {
	...
	try {
		...
		if (isGlobal) {
			...
			return regex.replace(target, (match, ...groups) =>
				processGensub(replacement, match as string, groups.slice(0, -2) as string[]),
			);
		} else {
			let count = 0;
			const regex = createUserRegex(pattern, "g");
			return regex.replace(target, (match, ...groups) => {
				...
				if (count === occurrenceNum) {
					return processGensub(replacement, match as string, groups.slice(0, -2) as string[]);
				}
...
function processGensub(replacement: string, match: string, groups: string[]): string {
	let result = "";
	let i = 0;
	while (i < replacement.length) {
		if (replacement[i] === "\\" && i + 1 < replacement.length) {
...
async function awkTolower(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<string> {
	if (args.length === 0) return "";
	return toAwkString(await evaluator.evalExpr(args[0])).toLowerCase();
}

async function awkToupper(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<string> {
	if (args.length === 0) return "";
	return toAwkString(await evaluator.evalExpr(args[0])).toUpperCase();
}

async function awkSprintf(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<string> {
	if (args.length === 0) return "";
...
async function awkInt(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length === 0) return 0;
	return Math.floor(toNumber(await evaluator.evalExpr(args[0])));
}

async function awkSqrt(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length === 0) return 0;
	return Math.sqrt(toNumber(await evaluator.evalExpr(args[0])));
}

async function awkSin(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length === 0) return 0;
	return Math.sin(toNumber(await evaluator.evalExpr(args[0])));
}

async function awkCos(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length === 0) return 0;
	return Math.cos(toNumber(await evaluator.evalExpr(args[0])));
}

async function awkAtan2(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
...
async function awkLog(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length === 0) return 0;
	return Math.log(toNumber(await evaluator.evalExpr(args[0])));
}

async function awkExp(args: AwkExpr[], _ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
	if (args.length === 0) return 1;
	return Math.exp(toNumber(await evaluator.evalExpr(args[0])));
}

function awkRand(_args: AwkExpr[], ctx: AwkRuntimeContext, _evaluator: AwkEvaluator): number {
	return ctx.random ? ctx.random() : Math.random();
}

async function awkSrand(args: AwkExpr[], ctx: AwkRuntimeContext, evaluator: AwkEvaluator): Promise<number> {
		...
		if (format[i] === "%" && i + 1 < format.length) {
			...
			const skipLengthMods = () => {
				if (j < format.length) {
					...
					if (
						j + 1 < format.length &&
						((format[j] === "h" && format[j + 1] === "h") || (format[j] === "l" && format[j + 1] === "l"))
					) {
						j += 2;
						return;
					}
