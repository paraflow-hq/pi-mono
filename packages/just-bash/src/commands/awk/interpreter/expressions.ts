...
import type { AwkArrayAccess, AwkExpr, AwkFieldRef, AwkFunctionDef, AwkVariable } from "../ast.js";
 ...
 */
export async function evalExpr(ctx: AwkRuntimeContext, expr: AwkExpr): Promise<AwkValue> {
...
async function evalFieldRef(ctx: AwkRuntimeContext, expr: AwkFieldRef): Promise<AwkValue> {
	const index = Math.floor(toNumber(await evalExpr(ctx, expr.index)));
	return getField(ctx, index);
}

async function evalArrayAccess(ctx: AwkRuntimeContext, expr: AwkArrayAccess): Promise<AwkValue> {
...
async function evalBinaryOp(
	...
	if (op === "~") {
		...
		const pattern = expr.right.type === "regex" ? expr.right.pattern : toAwkString(await evalExpr(ctx, expr.right));
		try {
...
async function evalUnaryOp(ctx: AwkRuntimeContext, expr: { operator: string; operand: AwkExpr }): Promise<AwkValue> {
...
async function evalFunctionCall(ctx: AwkRuntimeContext, name: string, args: AwkExpr[]): Promise<AwkValue> {
...
async function callUserFunction(ctx: AwkRuntimeContext, func: AwkFunctionDef, args: AwkExpr[]): Promise<AwkValue> {
...
async function evalInExpr(ctx: AwkRuntimeContext, key: AwkExpr, array: string): Promise<AwkValue> {
	let keyStr: string;
	if (key.type === "tuple") {
...
async function evalTuple(ctx: AwkRuntimeContext, elements: AwkExpr[]): Promise<AwkValue> {
