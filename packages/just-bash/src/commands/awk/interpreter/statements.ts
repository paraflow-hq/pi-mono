 ...
 */
export async function executeBlock(ctx: AwkRuntimeContext, statements: AwkStmt[]): Promise<void> {
 ...
 */
async function executeStmt(ctx: AwkRuntimeContext, stmt: AwkStmt): Promise<void> {
	...
	switch (stmt.type) {
		...
		case "exit":
			ctx.shouldExit = true;
			ctx.exitCode = stmt.code ? Math.floor(toNumber(await evalExpr(ctx, stmt.code))) : 0;
			break;
 ...
 * Execute while loop.
 */
async function executeWhile(ctx: AwkRuntimeContext, stmt: { condition: AwkExpr; body: AwkStmt }): Promise<void> {
	let iterations = 0;
 ...
 */
async function executeDoWhile(ctx: AwkRuntimeContext, stmt: { body: AwkStmt; condition: AwkExpr }): Promise<void> {
	let iterations = 0;
 ...
 */
async function executeDelete(ctx: AwkRuntimeContext, target: AwkArrayAccess | AwkVariable): Promise<void> {
