...
import type { AwkArrayAccess, AwkExpr, AwkFieldRef, AwkStmt, AwkVariable } from "./ast.js";
 ...
 */
function parsePrintAssignment(p: PrintParserContext, allowGt: boolean): AwkExpr {
...
function parsePrintMatch(p: PrintParserContext): AwkExpr {
	...
	while (p.match(TokenTypes.MATCH as TokenType, TokenTypes.NOT_MATCH as TokenType)) {
		const op = p.advance().type === (TokenTypes.MATCH as TokenType) ? "~" : "!~";
