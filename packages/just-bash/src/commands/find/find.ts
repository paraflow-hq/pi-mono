...
import type { Command, CommandContext, ExecResult, TraceCallback } from "../../types.js";
...
function emitTraceSummary(trace: TraceCallback, counters: TraceCounters, totalMs: number): void {
	trace({
		category: "find",
		name: "summary",
		durationMs: totalMs,
...
export const findCommand: Command = {
	...
	async execute(args: string[], ctx: CommandContext): Promise<ExecResult> {
		...
		for (let searchPath of searchPaths) {
			...
			async function processNode(item: WorkItem): Promise<ProcessedNode | null> {
				...
				const inTerminalDir =
					pathPruningHints.terminalDirName !== null && name === pathPruningHints.terminalDirName;
					...
					if (hasReaddirWithFileTypes && ctx.fs.readdirWithFileTypes) {
						...
						if (shouldDescendIntoSubdirs) {
							...
							children = entriesWithTypes
								.filter((entry) => entry.isFile && (!extFilter || entry.name.endsWith(extFilter)))
								.map((entry, idx) => ({
									path: currentPath === "/" ? `/${entry.name}` : `${currentPath}/${entry.name}`,
									depth: depth + 1,
									typeInfo: {
			...
			async function findIterative(): Promise<NodeResult> {
				...
				if (depthFirst) {
					...
					const nodeResults: Map<number, { path: string; printfData: FindResult | null }> = new Map();
					let orderCounter = 0;
