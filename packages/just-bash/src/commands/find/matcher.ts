 ...
 */
export function evaluateExpressionWithPrune(expr: Expression, ctx: EvalContext): EvalResult {
	switch (expr.type) {
		case "name": {
		...
		case "type":
			if (expr.fileType === "f") return { matches: ctx.isFile, pruned: false, printed: false };
			if (expr.fileType === "d") return { matches: ctx.isDirectory, pruned: false, printed: false };
			return { matches: false, pruned: false, printed: false };
		case "empty":
		...
		case "newer": {
			...
			if (refMtime === undefined) return { matches: false, pruned: false, printed: false };
 ...
 */
export function extractPathPruningHints(expr: Expression | null): PathPruningHint {
...
export function evaluateSimpleExpression(
	...
	switch (expr.type) {
		...
		case "type":
			if (expr.fileType === "f") return { matches: isFile, pruned: false, printed: false };
			if (expr.fileType === "d") return { matches: isDirectory, pruned: false, printed: false };
			return { matches: false, pruned: false, printed: false };
		case "prune":
			return { matches: true, pruned: true, printed: false };
		case "print":
 ...
 */
export function evaluateForEarlyPrune(expr: Expression | null, ctx: EarlyEvalContext): { shouldPrune: boolean } {
 ...
 */
function evaluatePruneBranchEarly(expr: Expression, ctx: EarlyEvalContext): { shouldPrune: boolean } {
	switch (expr.type) {
		case "or": {
