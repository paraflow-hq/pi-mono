...
export const AST = {
	...
	parameterExpansion(parameter: string, operation: ParameterOperation | null = null): ParameterExpansionPart {
		return { type: "ParameterExpansion", parameter, operation };
	},

	commandSubstitution(body: ScriptNode, legacy = false): CommandSubstitutionPart {
	...
	assignment(name: string, value: WordNode | null, append = false, array: WordNode[] | null = null): AssignmentNode {
	...
	hereDoc(delimiter: string, content: WordNode, stripTabs = false, quoted = false): HereDocNode {
		return { type: "HereDoc", delimiter, content, stripTabs, quoted };
	},

	ifNode(clauses: IfClause[], elseBody: StatementNode[] | null = null, redirections: RedirectionNode[] = []): IfNode {
