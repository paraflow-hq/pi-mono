...
import type { DefenseInDepthConfig, SecurityViolation, SecurityViolationType } from "./types.js";
...
export class WorkerDefenseInDepth {
	 ...
	 */
	private recordViolation(type: SecurityViolationType, path: string, message: string): SecurityViolation {
	 ...
	 */
	private protectConstructorChain(excludeTypes: Set<SecurityViolationType>): void {
		...
		let AsyncGeneratorFunction: (new (...args: unknown[]) => unknown) | null = null;
	 ...
	 */
	private patchPrototypeConstructor(prototype: object, path: string, violationType: SecurityViolationType): void {
		const self = this;
		const auditMode = this.config.auditMode;
	...
	private protectModuleLoad(): void {
		...
		try {
			...
			if (typeof process !== "undefined") {
				const mainModule = (process as unknown as Record<string, unknown>).mainModule;
				if (mainModule && typeof mainModule === "object") {
					ModuleClass = (mainModule as unknown as Record<string, unknown>).constructor as unknown as Record<
						string,
						unknown
					>;
				}
			...
			if (!ModuleClass && typeof require !== "undefined" && typeof require.main !== "undefined") {
				ModuleClass = (require.main as unknown as Record<string, unknown>).constructor as unknown as Record<
					string,
					unknown
				>;
			}
			...
			if (strategy === "freeze") {
				...
				const proxy =
					typeof original === "function"
						? this.createBlockingProxy(original as (...args: unknown[]) => unknown, path, violationType)
						: this.createBlockingObjectProxy(original as object, path, violationType);
