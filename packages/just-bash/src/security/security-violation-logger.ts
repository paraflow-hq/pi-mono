...
export class SecurityViolationLogger {
	...
	private violationsByType: Map<SecurityViolationType, SecurityViolation[]> = new Map();
	...
	record(violation: SecurityViolation): void {
		...
		const processedViolation = this.options.includeStackTraces ? violation : { ...violation, stack: undefined };
