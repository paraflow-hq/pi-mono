...
export function createSecureFetch(config: NetworkConfig): SecureFetch {
	 ...
	 */
	async function secureFetch(url: string, options: SecureFetchOptions = {}): Promise<FetchResult> {
		...
		const effectiveTimeout = options.timeoutMs !== undefined ? Math.min(options.timeoutMs, timeoutMs) : timeoutMs;
 ...
 */
async function responseToResult(response: Response, url: string, maxResponseSize: number): Promise<FetchResult> {
