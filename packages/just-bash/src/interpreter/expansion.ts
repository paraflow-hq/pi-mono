 ...
 */
function checkStringLength(str: string, maxLength: number, context: string): void {
...
import { expandWordWithGlobImpl, type WordGlobExpansionDeps } from "./expansion/word-glob-expansion.js";
...
export async function expandWord(ctx: InterpreterContext, word: WordNode): Promise<string> {
 ...
 */
export async function expandWordForRegex(ctx: InterpreterContext, word: WordNode): Promise<string> {
	const parts: string[] = [];
	for (const part of word.parts) {
 ...
 */
export async function expandWordForPattern(ctx: InterpreterContext, word: WordNode): Promise<string> {
	const parts: string[] = [];
	for (const part of word.parts) {
		if (part.type === "Escaped") {
 ...
 */
async function expandWordForGlobbing(ctx: InterpreterContext, word: WordNode): Promise<string> {
	const parts: string[] = [];
	for (const part of word.parts) {
...
function hasBraceExpansion(parts: WordPart[]): boolean {
	for (const part of parts) {
		...
		if (part.type === "DoubleQuoted" && hasBraceExpansion(part.parts)) return true;
	}
	return false;
}
 ...
 */
async function expandWordWithBracesAsync(ctx: InterpreterContext, word: WordNode): Promise<string[]> {
...
export function hasQuotedMultiValueAt(ctx: InterpreterContext, word: WordNode): boolean {
...
async function expandWordAsync(ctx: InterpreterContext, word: WordNode): Promise<string> {
	const wordParts = word.parts;
...
async function expandPart(ctx: InterpreterContext, part: WordPart, inDoubleQuotes = false): Promise<string> {
