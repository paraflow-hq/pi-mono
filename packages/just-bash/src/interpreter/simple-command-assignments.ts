 ...
 */
export async function processAssignments(ctx: InterpreterContext, node: SimpleCommandNode): Promise<AssignmentResult> {
...
async function processSubscriptAssignment(
	...
	if (node.name) {
		...
		if (localDepth !== undefined && localDepth === ctx.state.callDepth && ctx.state.localScopes.length > 0) {
...
async function processScalarAssignment(
	...
	// Resolve nameref
	let targetName = name;
	let namerefArrayRef: { arrayName: string; subscriptExpr: string } | null = null;
