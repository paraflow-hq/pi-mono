 ...
 */
export async function expandLocalArrayAssignment(ctx: InterpreterContext, word: WordNode): Promise<string | null> {
	// First, join all parts to check if this looks like an array assignment
	const fullLiteral = word.parts.map((p) => (p.type === "Literal" ? p.value : "\x00")).join("");
 ...
 */
export async function expandScalarAssignmentArg(ctx: InterpreterContext, word: WordNode): Promise<string | null> {
