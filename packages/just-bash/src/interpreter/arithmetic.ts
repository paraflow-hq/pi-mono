 ...
 */
function applyAssignmentOp(current: number, value: number, operator: string): number {
	switch (operator) {
		case "=":
			return value;
		case "+=":
 ...
 */
async function getArithVariable(ctx: InterpreterContext, name: string): Promise<string> {
 ...
 */
async function evaluateArithValue(ctx: InterpreterContext, value: string): Promise<number> {
	if (!value) {
		return 0;
	}
 ...
 */
async function expandBracedContent(ctx: InterpreterContext, content: string): Promise<string> {
	// Handle ${#var} - length
