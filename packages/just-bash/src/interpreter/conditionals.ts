...
export async function evaluateConditional(ctx: InterpreterContext, expr: ConditionalExpressionNode): Promise<boolean> {
	switch (expr.type) {
		case "CondBinary": {
...
export async function evaluateTestArgs(ctx: InterpreterContext, args: string[]): Promise<ExecResult> {
	if (args.length === 0) {
		return execResult("", "", 1);
	}
	...
	if (args.length === 3) {
		...
		if (left === "!") {
			...
			return execResult(
				"",
				negResult.stderr,
				negResult.exitCode === 0 ? 1 : negResult.exitCode === 1 ? 0 : negResult.exitCode,
			);
		}
...
export function matchPattern(value: string, pattern: string, nocasematch = false, extglob = false): boolean {
 ...
 */
function computePatternLength(pattern: string, extglob: boolean): number | null {
	let length = 0;
	let i = 0;
 ...
 */
async function evalArithExpr(ctx: InterpreterContext, expr: string): Promise<number> {
	expr = expr.trim();
	if (expr === "") return 0;
 ...
 */
function convertPosixCharClass(pattern: string, startIndex: number): { converted: string; endIndex: number } {
	let i = startIndex + 1;
	let result = "[";
