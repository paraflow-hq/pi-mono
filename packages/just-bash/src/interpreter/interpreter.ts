...
import { type BuiltinDispatchContext, dispatchBuiltin, executeExternalCommand } from "./builtin-dispatch.js";
...
import { failure, OK, result, testResult, throwExecutionLimit } from "./helpers/result.js";
...
export interface InterpreterOptions {
	...
	exec: (script: string, options?: { env?: Record<string, string>; cwd?: string }) => Promise<ExecResult>;
...
export class Interpreter {
	 ...
	 */
	private async executeUserScript(scriptPath: string, args: string[], stdin = ""): Promise<ExecResult> {
	...
	private async executeSimpleCommand(node: SimpleCommandNode, stdin: string): Promise<ExecResult> {
		try {
	...
	private async executeSimpleCommandInner(node: SimpleCommandNode, stdin: string): Promise<ExecResult> {
		...
		const isLiteralAssignmentBuiltinForExport =
			node.name && isWordLiteralMatch(node.name, ["local", "declare", "typeset", "export", "readonly"]);
		...
		const isLiteralAssignmentBuiltin =
			isWordLiteralMatch(node.name, ["local", "declare", "typeset", "export", "readonly"]) &&
			(commandName === "local" ||
				commandName === "declare" ||
				commandName === "typeset" ||
				commandName === "export" ||
		...
		if (!commandName) {
			const isOnlyExpansions = node.name.parts.every(
				(p) =>
					p.type === "CommandSubstitution" || p.type === "ParameterExpansion" || p.type === "ArithmeticExpansion",
			);
			if (isOnlyExpansions) {
		...
		if (args.length > 0) {
			...
			if (
				(commandName === "declare" || commandName === "local" || commandName === "typeset") &&
		...
		const isPosixSpecialWithPersistence =
			isPosixSpecialBuiltin(commandName) && commandName !== "unset" && commandName !== "eval";
	...
	private async executeSubshell(node: SubshellNode, stdin = ""): Promise<ExecResult> {
	...
	private async executeArithmeticCommand(node: ArithmeticCommandNode): Promise<ExecResult> {
		...
		try {
			...
			const bodyResult = failure(`bash: arithmetic expression: ${(error as Error).message}\n`);
			return applyRedirections(this.ctx, bodyResult, node.redirections);
		}
	}
	...
	private async executeConditionalCommand(node: ConditionalCommandNode): Promise<ExecResult> {
		...
		try {
			...
			const bodyResult = failure(`bash: conditional expression: ${(error as Error).message}\n`, exitCode);
			return applyRedirections(this.ctx, bodyResult, node.redirections);
		}
	}
}
