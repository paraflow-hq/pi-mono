...
export function handleShopt(ctx: InterpreterContext, args: string[]): ExecResult {
	// Parse arguments
	...
	if (optionNames.length === 0) {
		...
		for (const opt of SHOPT_OPTIONS) {
			const value = ctx.state.shoptOptions[opt];
			output.push(printFlag ? `shopt ${value ? "-s" : "-u"} ${opt}` : `${opt}\t\t${value ? "on" : "off"}`);
		}
		return {
			exitCode: 0,
			stdout: `${output.join("\n")}\n`,
...
function handleSetOptions(
	...
	if (optionNames.length === 0) {
		...
		for (const opt of ALL_SET_OPTIONS) {
			...
			output.push(printFlag ? `set ${value ? "-o" : "+o"} ${opt}` : `${opt}\t\t${value ? "on" : "off"}`);
		}
		return {
			exitCode: 0,
			stdout: output.length > 0 ? `${output.join("\n")}\n` : "",
