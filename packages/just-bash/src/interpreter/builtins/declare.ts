 ...
 */
export function applyCaseTransform(ctx: InterpreterContext, name: string, value: string): string {
 ...
 */
async function evaluateIntegerValue(ctx: InterpreterContext, value: string): Promise<string> {
	try {
 ...
 */
function parseArrayAssignment(arg: string): { name: string; indexExpr: string; value: string } | null {
...
export async function handleDeclare(ctx: InterpreterContext, args: string[]): Promise<ExecResult> {
	// Parse flags
	...
	for (const arg of processedArgs) {
		...
		if (arg.includes("=")) {
			...
			const hasArrayElements = Array.from(ctx.state.env.keys()).some(
				(key) => key.startsWith(`${name}_`) && !key.startsWith(`${name}__length`),
			);
 ...
 */
export async function handleReadonly(ctx: InterpreterContext, args: string[]): Promise<ExecResult> {
	// Parse flags
