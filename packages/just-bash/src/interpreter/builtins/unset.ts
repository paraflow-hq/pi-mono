...
function isQuotedStringIndex(indexExpr: string): boolean {
	...
	return (
		(indexExpr.startsWith("'") && indexExpr.endsWith("'")) || (indexExpr.startsWith('"') && indexExpr.endsWith('"'))
	);
}
 ...
 */
async function evaluateArrayIndex(ctx: InterpreterContext, indexExpr: string): Promise<number | null> {
 ...
 */
async function expandAssocSubscript(ctx: InterpreterContext, subscriptExpr: string): Promise<string> {
...
export async function handleUnset(ctx: InterpreterContext, args: string[]): Promise<ExecResult> {
