 ...
 */
function encodeRwFdContent(path: string, position: number, content: string): string {
	return `__rw__:${path.length}:${path}:${position}:${content}`;
}

export function handleRead(ctx: InterpreterContext, args: string[], stdin: string, stdinSourceFd = -1): ExecResult {
	// Parse options
	let raw = false;
	let delimiter = "\n";
	let _prompt = "";
	...
	const parseOption = (opt: string, argIndex: number): { nextArgIndex: number } => {
		let j = 1; // skip the '-'
		while (j < opt.length) {
			const ch = opt[j];
			if (ch === "r") {
