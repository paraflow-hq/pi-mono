 ...
 */
export function splitByIfsForExpansionEx(value: string, ifs: string): IfsExpansionSplitResult {
	// Empty IFS means no splitting
	if (ifs === "") {
		return {
			words: value ? [value] : [],
 ...
 */
export function stripTrailingIfsWhitespace(value: string, ifs: string, raw?: boolean): string {
	if (ifs === "") return value;
	const { whitespace, nonWhitespace } = categorizeIfs(ifs);
