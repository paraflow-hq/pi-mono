 ...
 */
export function getArrayIndices(ctx: InterpreterContext, arrayName: string): number[] {
 ...
 */
export function getAssocArrayKeys(ctx: InterpreterContext, arrayName: string): string[] {
	const prefix = `${arrayName}_`;
...
export function parseKeyedElementFromWord(word: WordNode): ParsedKeyedElement | null {
	...
	if (second.type === "Literal" && second.value.startsWith("]")) {
		...
		if (afterBracket.startsWith("+=") || afterBracket.startsWith("=")) {
			...
			if (!third.value.startsWith("=") && !third.value.startsWith("+=")) return null;
			key = first.pattern.slice(1);
			secondPart = third;
			secondPartIndex = 2;
		} else {
		...
		if (!second.value.startsWith("=") && !second.value.startsWith("+=")) return null;
