...
async function handleArrayExpansionCases(
	...
	{
		const arrayPatternPrefixSuffixResult = await handleArrayPatternWithPrefixSuffix(
			ctx,
			wordParts,
			hasArrayAtExpansion,
			deps.expandPart,
...
async function handleUnquotedExpansionCases(
	...
	{
		const unquotedArrayPatReplResult = await handleUnquotedArrayPatternReplacement(
			ctx,
			wordParts,
			deps.expandWordPartsAsync,
			deps.expandPart,
	...
	{
		const unquotedPosPatRemResult = await handleUnquotedPositionalPatternRemoval(
			ctx,
			wordParts,
			deps.expandWordPartsAsync,
			deps.expandPart,
	...
	{
		const unquotedPrefixSuffixResult = await handleUnquotedPositionalWithPrefixSuffix(
			ctx,
			wordParts,
			deps.expandPart,
		);
...
async function expandDoubleQuotedWithWordProducing(
	...
	return [prefix + values[0], ...values.slice(1, -1), values[values.length - 1] + suffix];
}
 ...
 */
async function expandGlobPattern(ctx: InterpreterContext, pattern: string): Promise<string[]> {
