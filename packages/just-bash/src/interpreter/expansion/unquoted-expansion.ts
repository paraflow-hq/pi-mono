...
import type { ArithExpr, ParameterExpansionPart, SubstringOp, WordNode, WordPart } from "../../ast/types.js";
 ...
 */
async function applyGlobExpansion(ctx: InterpreterContext, words: string[]): Promise<string[]> {
...
export async function handleUnquotedArrayPatternReplacement(
	...
	const paramPart = wordParts[unquotedArrayPatReplIdx] as ParameterExpansionPart;
	...
	const replacement = operation.replacement ? await expandWordPartsAsync(ctx, operation.replacement.parts) : "";
...
export async function handleUnquotedPositionalSlicing(
	...
	const offset = operation.offset ? await evaluateArithmetic(ctx, operation.offset.expression) : 0;
	const length = operation.length ? await evaluateArithmetic(ctx, operation.length.expression) : undefined;
 ...
 */
export function handleUnquotedVarNamePrefix(ctx: InterpreterContext, wordParts: WordPart[]): UnquotedExpansionResult {
	if (
		wordParts.length !== 1 ||
		wordParts[0].type !== "ParameterExpansion" ||
 ...
 */
export function handleUnquotedArrayKeys(ctx: InterpreterContext, wordParts: WordPart[]): UnquotedExpansionResult {
	if (
		wordParts.length !== 1 ||
		wordParts[0].type !== "ParameterExpansion" ||
