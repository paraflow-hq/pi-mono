 ...
 */
export async function expandDollarVarsInArithText(ctx: InterpreterContext, text: string): Promise<string> {
	let result = "";
	let i = 0;
	while (i < text.length) {
		if (text[i] === "$") {
 ...
 */
export async function expandSubscriptForAssocArray(ctx: InterpreterContext, subscript: string): Promise<string> {
