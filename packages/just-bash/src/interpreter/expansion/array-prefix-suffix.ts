...
import type { PatternRemovalOp, PatternReplacementOp, WordNode, WordPart } from "../../ast/types.js";
...
export async function handleArrayPatternWithPrefixSuffix(
	...
	for (let i = 0; i < dqPart.parts.length; i++) {
		...
		if (
			...
			(p.operation?.type === "PatternRemoval" || p.operation?.type === "PatternReplacement")
		) {
	...
	if (arrayOperation?.type === "PatternRemoval") {
		...
		const replacement = op.replacement ? await expandWordPartsAsync(ctx, op.replacement.parts) : "";
	...
	const result = [prefix + values[0], ...values.slice(1, -1), values[values.length - 1] + suffix];
