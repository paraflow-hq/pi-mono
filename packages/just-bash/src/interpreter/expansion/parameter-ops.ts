 ...
 */
export function handleLength(ctx: InterpreterContext, parameter: string, value: string): string {
...
export async function handleCaseModification(
	...
	if (operation.pattern) {
		...
		const transform =
			operation.direction === "upper" ? (c: string) => c.toUpperCase() : (c: string) => c.toLowerCase();
 ...
 */
export function handleArrayKeys(ctx: InterpreterContext, operation: { array: string; star: boolean }): string {
 ...
 */
export function handleVarNamePrefix(ctx: InterpreterContext, operation: { prefix: string; star: boolean }): string {
