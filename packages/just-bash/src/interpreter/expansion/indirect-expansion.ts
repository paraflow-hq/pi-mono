...
export async function handleIndirectArrayExpansion(
	...
	if (indirOp.innerOp) {
		...
		if (indirOp.innerOp.type === "Transform" && (indirOp.innerOp as { operator: string }).operator === "a") {
...
async function handleIndirectArraySlicing(
	...
	const length = innerOp.length ? await evaluateArithmetic(ctx, innerOp.length.expression) : undefined;
...
export async function handleIndirectInAlternative(
	...
	if (
		...
		(wordParts[0].operation?.type !== "UseAlternative" && wordParts[0].operation?.type !== "DefaultValue")
	) {
		return null;
	}
