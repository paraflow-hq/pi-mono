 ...
 */
function expandSimpleVarsInSubscript(ctx: InterpreterContext, subscript: string): string {
 ...
 */
export function getArrayElements(ctx: InterpreterContext, arrayName: string): Array<[number | string, string]> {
	...
	if (isAssoc) {
		...
		return keys.map((key) => [key, ctx.state.env.get(`${arrayName}_${key}`) ?? ""]);
	}

	// For indexed arrays, get numeric indices
	const indices = getArrayIndices(ctx, arrayName);
	return indices.map((index) => [index, ctx.state.env.get(`${arrayName}_${index}`) ?? ""]);
}
...
export async function getVariable(
	...
	if (isNameref(ctx, name)) {
		...
		if ((value === undefined || value === "") && checkNounset && ctx.state.options.nounset) {
			throw new NounsetError(name);
		}
		return value || "";
	}
	...
	if (value !== undefined) {
		...
		if (ctx.state.tempEnvBindings?.some((b) => b.has(name))) {
			ctx.state.accessedTempEnvVars = ctx.state.accessedTempEnvVars || new Set();
 ...
 */
export async function isVariableSet(ctx: InterpreterContext, name: string): Promise<boolean> {
