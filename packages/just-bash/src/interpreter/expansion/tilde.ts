 ...
 */
export function applyTildeExpansion(ctx: InterpreterContext, value: string): string {
	if (!value.startsWith("~")) {
		return value;
	}
	...
	const home = ctx.state.env.get("HOME") !== undefined ? ctx.state.env.get("HOME") : "/home/user";
