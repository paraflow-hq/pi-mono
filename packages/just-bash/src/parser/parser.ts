...
export class Parser {
	...
	private checkUnexpectedToken(): StatementNode | null {
		...
		if (t === TokenType.RBRACE || t === TokenType.RPAREN) {
			...
			return AST.statement([AST.pipeline([AST.simpleCommand(null, [], [], [])])], [], false, {
				message: errorMsg,
				token: v,
			});
		}
	...
	parseCommandSubstitution(value: string, start: number): { part: CommandSubstitutionPart; endIndex: number } {
	...
	parseArithmeticExpansion(value: string, start: number): { part: ArithmeticExpansionPart; endIndex: number } {
		// Skip $((
	...
	private parseCompoundCommandBody(options?: { forFunctionBody?: boolean }): CompoundCommandNode {
