 ...
 */
export const ARITH_ASSIGN_OPS = ["=", "+=", "-=", "*=", "/=", "%=", "<<=", ">>=", "&=", "|=", "^="] as const;
...
export function parseNestedArithmetic(
	parseArithExpr: (p: Parser, input: string, pos: number) => { expr: ArithExpr; pos: number },
	p: Parser,
	input: string,
	currentPos: number,
 ...
 */
export function parseAnsiCQuoting(input: string, currentPos: number): { expr: ArithExpr; pos: number } | null {
 ...
 * Returns the numeric value of the string content
 */
export function parseLocalizationQuoting(input: string, currentPos: number): { expr: ArithExpr; pos: number } | null {
	if (input.slice(currentPos, currentPos + 2) !== '$"') {
		return null;
	}
