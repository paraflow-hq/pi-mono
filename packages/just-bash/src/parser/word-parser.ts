...
export function findMatchingBracket(_p: Parser, value: string, start: number, open: string, close: string): number {
	let depth = 1;
	let i = start + 1;
...
export function findParameterOperationEnd(_p: Parser, value: string, start: number): number {
	let i = start;
	let depth = 1;
...
export function findPatternEnd(_p: Parser, value: string, start: number): number {
	let i = start;
...
export function parseGlobPattern(_p: Parser, value: string, start: number): { pattern: string; endIndex: number } {
	let i = start;
	let pattern = "";
...
export function parseAnsiCQuoted(_p: Parser, value: string, start: number): { part: WordPart; endIndex: number } {
	let result = "";
	let i = start;
...
export function parseArithExprFromString(p: Parser, str: string): ArithmeticExpressionNode {
				...
				items: [
					{
						...
						step: rangeMatch[3] ? Number.parseInt(rangeMatch[3], 10) : undefined,
				...
				items: [
					{
						...
						step: charRangeMatch[3] ? Number.parseInt(charRangeMatch[3], 10) : undefined,
					},
				],
			},
			endIndex: closeIdx + 1,
...
export function tokenToRedirectOp(_p: Parser, type: TokenType): RedirectionOperator {
