...
function parseParameterOperation(
	...
	if (char === ":") {
		...
		return {
			operation: {
				...
				length: lengthStr !== null ? WordParser.parseArithExprFromString(p, lengthStr) : null,
			},
			endIndex: wordEnd,
		};
	}
	...
	if (char === "@" && /[QPaAEKkuUL]/.test(nextChar)) {
		const operator = nextChar as "Q" | "P" | "a" | "A" | "E" | "K" | "k" | "u" | "U" | "L";
		return {
			operation: {
				type: "Transform",
				operator,
...
function parseDoubleQuoted(p: Parser, value: string, start: number): { part: WordPart; endIndex: number } {
	...
	while (i < value.length) {
		...
		if (char === "~") {
			...
			if (i === 0 || prevChar === "=" || canExpandAfterColon) {
				...
				if (afterTilde === undefined || afterTilde === "/" || afterTilde === ":") {
					flushLiteral();
