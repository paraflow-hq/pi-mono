...
export class GlobExpander {
	...
	async expandArgs(args: string[], quotedFlags?: boolean[]): Promise<string[]> {
		...
		const expansionPromises: (Promise<string[]> | null)[] = args.map((arg, i) => {
	 ...
	 */
	private async expandSegments(fsPath: string, resultPrefix: string, segments: string[]): Promise<string[]> {
	 ...
	 */
	private async walkDirectoryMultiGlobstar(dir: string, subPattern: string, results: string[]): Promise<void> {
		...
		try {
			...
			if (this.fs.readdirWithFileTypes) {
				...
				for (let i = 0; i < entries.length; i += DEFAULT_BATCH_SIZE) {
					...
					entryInfos.push(...(batchResults.filter((r) => r !== null) as EntryInfo[]));
				}
